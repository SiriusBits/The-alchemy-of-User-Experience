...
ReactDOM.render(
  <AppContainer errorReporter={CustomErrorReporter}>
    <Deck progress="none" transition={[creeperTransition]} transitionDuration={500} theme={theme}>
      {slides.map((S, i) => {
...
if (module.hot) {
  module.hot.accept(() => {
    ...
    ReactDOM.render(
      <AppContainer errorReporter={CustomErrorReporter}>
        <Deck progress="none" transition={[creeperTransition]} transitionDuration={500} theme={newTheme}>
          {newSlides.map((S, i) => {
            let transition = transitions[i] || null;
            return <S transition={transition} key={`slide-${i}`} />;
          })}
